...
Try {
	...
	[string]$appVendor = 'The Nature Conservancy'
	[string]$appName = 'Indicators of Hydrologic Alteration'
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		Show-InstallationWelcome -CloseApps 'IHA7' -CheckDiskSpace -PersistPrompt
		...
		If (Test-Path "$env:Public\Desktop\IHA7.lnk") {
			Write-Log -Message "Deleting desktop shortcut..." -Source 'Pre-Installation' -LogType 'CMTrace'
			Remove-Item "$env:Public\Desktop\IHA7.lnk" -Force
		}
		Else {
			Write-Log -Message "Desktop shortcut not detected." -Source 'Pre-Installation' -LogType 'CMTrace'
		}
		## Delete the start menu shortcut
		If (Test-Path "$env:ProgramData\Microsoft\Windows\Start Menu\Programs\IHA7.lnk") {
			Write-Log -Message "Deleting start menu shortcut..." -Source 'Pre-Installation' -LogType 'CMTrace'
			Remove-Item "$env:ProgramData\Microsoft\Windows\Start Menu\Programs\IHA7.lnk" -Force
		}
		Else {
			Write-Log -Message "Start menu shortcut not detected." -Source 'Pre-Installation' -LogType 'CMTrace'
		}
		## Delete the installation
		If (Test-Path "$envProgramFiles\IHA71_En") {
			Write-Log -Message "Deleting installation files..." -Source 'Pre-Installation' -LogType 'CMTrace'
			Get-ChildItem -Path "$envProgramFiles\IHA71_En" -Recurse | Remove-Item -force -recurse
			Remove-Item "$envProgramFiles\IHA71_En" -Force
		}
		Else {
			Write-Log -Message "IHA7 installation files not detected." -Source 'Pre-Installation' -LogType 'CMTrace'
		}
		...
		Copy-Item -Path "$dirFiles\IHA71_En" -Destination $envProgramFiles -Recurse
		Write-Log -Message "Copying desktop shortcut..." -Source 'Installation' -LogType 'CMTrace'
		Copy-Item -Path "$dirFiles\IHA7.lnk" -Destination "$env:Public\Desktop"
		Write-Log -Message "Copying start menu shortcut..." -Source 'Installation' -LogType 'CMTrace'
		Copy-Item -Path "$dirFiles\IHA7.lnk" -Destination "$env:ProgramData\Microsoft\Windows\Start Menu\Programs"
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Show-InstallationWelcome -CloseApps 'IHA7' -CloseAppsCountdown 60
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU7QLUIclyYVpok2pZobTdmEFq
# W6agghHXMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# MRYEFGvswabmWsr4N9MTX3WXGG8yIME3MA0GCSqGSIb3DQEBAQUABIIBgASqcRhM
# ObpJqo1t0q9SR7odRhhqlhggNlK8f8SD4QwSoqAWCdyGFP3nf6Lj002fXvW0ov6o
# 2jPASAmEWvfR/o2YBjSQKvRhUHKH7z8zfkK5UpQotDZRKjVzv0wWWfvsxqMbkn8T
# MBGCPsitpHOwOUpLwV7l4vPDNn2xkUrZ6lgDZMMQH14O4si0ydgxqoDGxWw6j9mM
# 0kYzUWU82slGI6Pi+hXSBuvg0GGRsexyUrQVIjInG7W71lKyToTsHDP+ZsJZ/kgj
# 6bGGu5TNMV3kJczumWdEBaNFz0JVgAzg62u07qHU7zF64i5BNRly6Wb46XwPqPoI
# 72+YAx151olyexvEjXyeSG+kPp9PN2pXE109MT1xWGOFXF1FPjzaJXzW1tEzW0Xf
# bloFpHmgvOTfXAp6ZdEufQZ4l5dLsB1WSw1vZUz2h/FeeZ/Q17nM3JwE7ee4Hga6
# uSrTEkyCvS0+7AC3oa5W4iUmOBmFL6v1ef1jhAMo7jMG0+zMfudgQR8zTQ==
